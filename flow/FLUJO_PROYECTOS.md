...
- **Router + candado:** de `flow/ROUTER.md` el hook inyecta cada turno **solo el bloque marcado** (`NIMBUS:INYECTAR`) — plantilla del ESTATUS, escaleras de modelo y effort, y tabla trigger→escalón; el resto del archivo es el porqué y se lee bajo demanda. Antes de trabajo de proyecto, el candado obliga al Constructor a declarar al Director qué escalón(es), qué modelo y qué effort usa, para que apruebe o ajuste. La declaración va en el **bloque ESTATUS enmarcado al final** de cada respuesta (nunca omite áreas: veredicto + 💾 guardado + 🤖 modelo con `/model` + 🎚️ effort con medidor y `/effort` (+ sub-línea 🎭 roles del pipeline siempre debajo — 👷 directo por default) + escalones + medidor de contexto/sesión con `/compact`·`/clear` según banda; el hook calcula 💾 y el medidor, y reporta el modelo/effort que de verdad corrieron el turno pasado; formato en `flow/ROUTER.md` §Candado).
